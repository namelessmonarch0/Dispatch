...
/// TEMPORARY: compares the portable-pty spawn with this one on the Windows
/// runner, to find why a Git Bash pane's title changed. Reverted once read.
...
mod diagnosis {
    ...
    /// Everything read for `total`, with `input` typed after `after`.
    fn collect(
    ...
    fn ours(
        ...
        .expect("ours spawns");
        let out = collect(process.reader, process.writer, input, after, total);
        if let Some(pid) = process.pid {
            let _ = crate::process::terminate_tree(pid, Duration::ZERO);
        }
        out
    }
    ...
    fn theirs(
        ...
        let mut child = pair.slave.spawn_command(builder).expect("theirs spawns");
        let reader = pair.master.try_clone_reader().expect("reader");
        let mut writer = pair.master.take_writer().expect("writer");
        let _ = writer.write_all(b"\x1b[1;1R");
        let out = collect(reader, writer, input, after, total);
        let _ = child.kill();
        drop(pair.slave);
        out
    }

    fn vars(out: &[u8]) -> BTreeMap<String, String> {
        let text = String::from_utf8_lossy(out);
        let mut vars = BTreeMap::new();
        for line in text.split(['\r', '\n']) {
            // Strip escape sequences ConPTY paints around the text.
            ...
            if let Some((key, value)) = clean.split_once('=')
                ...
                && !key.contains(' ')
            {
                vars.insert(key.to_uppercase(), value.trim_end().to_string());
            }
        }
        vars
    }
    ...
    #[test]
    fn diagnose_the_two_spawns() {
        let set_args = vec!["/c".to_string(), "set".to_string()];
        let mine = vars(&ours(
            "cmd.exe",
            &set_args,
            b"",
        ...
        say(&format!(
            "DIAG env: ours has {} vars, portable-pty {}",
            mine.len(),
            old.len()
        ));
        for (key, value) in &old {
            match mine.get(key) {
                None => say(&format!("DIAG env only-old {key}={value}")),
                Some(v) if v != value => {
                    say(&format!("DIAG env differs {key}: ours={v:?} old={value:?}"))
                }
                _ => {}
            }
        }
        for (key, value) in &mine {
            if !old.contains_key(key) {
                say(&format!("DIAG env only-ours {key}={value}"));
            }
        }

        let where_args = vec!["/c".to_string(), "where sh".to_string()];
        let found = ours(
            "cmd.exe",
            &where_args,
            b"",
        ...
        say(&format!(
            "DIAG where sh: {}",
            String::from_utf8_lossy(&found).escape_debug()
        ));

        let probe = b"echo \"PROBE home=$HOME msystem=$MSYSTEM msys=$MSYS dash=$- zero=$0 bash=$BASH\"; shopt login_shell; echo \"PS1=$PS1\" | head -c 200; echo\r";
        let shell_ours = ours(
            "sh",
            &[],
            probe,
        ...
        say(&format!(
            "DIAG sh ours: {}",
            String::from_utf8_lossy(&shell_ours).escape_debug()
        ));
        let shell_old = theirs(
            "sh",
            &[],
            probe,
            Duration::from_secs(4),
        ...
        say(&format!(
            "DIAG sh old: {}",
            String::from_utf8_lossy(&shell_old).escape_debug()
        ));
    }
}
